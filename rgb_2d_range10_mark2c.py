...
def bmp_write(d,byte):
    ...
    #create the outfile
    outfile = open('bitmap_image2c.bmp','wb')
...
def main():
    ...
    def rand_color_mem2d(buddy_color = None):
        ...
        if last_color is not None:
            ...
            if buddy_color is not None:
                 if (buddy_color[0] > last_color[0]):
                    r = random.randint(last_color[0], buddy_color[0])
                 else:
                    r = random.randint(buddy_color[0], last_color[0])
            else:
            ...
            if buddy_color is not None:
                if (buddy_color[1] > last_color[1]):
                    g = random.randint(last_color[1], buddy_color[1])
                else:
                    g = random.randint(buddy_color[1], last_color[1])
            else:
            ...
            if buddy_color is not None:
                if (buddy_color[2] > last_color[2]):
                    b = random.randint(last_color[2], buddy_color[2])
                else:
                    b = random.randint(buddy_color[2], last_color[2])
            else:
    ...
    for row in range(d['height']-1,-1,-1):# (BMPs are L to R from the bottom L row)
        ...
        for column in range(d['width']):
            if (d['height'] - (row+1) > 0):
                try:
                    color = rand_color_mem2d(bitmapList[d['height'] - (row+1) + 2][column])
                except:
